...
const ALLOWED_DIRECTOR_IDS = [
  'miyazaki', 'wkw', 'koreeda', 'wes', 'nolan',
  'chow', 'jia', 'lee', 'kurosawa', 'coppola',
  'chazelle', 'tarantino', 'movie-custom', 'custom', 'grid9'
];
...
// 保存海报
const savePosterSchema = z.object({
...
const hotTopicKeywordSchema = z.object({
  keyword: z.string().min(1, '缺少关键词').max(64, '关键词过长').regex(
    /^[\u4e00-\u9fa5a-zA-Z0-9\s,.;:!?，。；：！？、'"《》\-_\u3000]+$/,
    '关键词包含非法字符'
  ),
});

...
import { DNA_DIMENSIONS as dims, DNA_VALUE_MAPS as valMaps, dnaToValues, drawDNAGrid, drawDNALabels } from './utils/dna.js';
...
const state = {
      ...
      creativeModalEl.addEventListener('zj:close', () => {
        ...
        if (activeTab) {
          switch (activeTab.dataset.creativeTab) {
            case 'quote': closeQuoteCard(); break;
            case 'scene': closeSceneRecreate(); break;
            case 'comic': closeComicStrip(); break;
            case 'meme': closeCharacterMeme(); break;
            case 'guess': closeGuessPoster(); break;
          }
        }
      });
    }
  ...
  function renderCarousel() {
    ...
    carousel.innerHTML = filtered.map(movie => {
      ...
      const heatBg = movie.heatScore >= HEAT_EXTREME_THRESHOLD ? 'rgba(231,76,60,.9)' : movie.heatScore >= HEAT_HOT_THRESHOLD ? 'rgba(230,126,34,.9)' : movie.heatScore >= HEAT_WARM_THRESHOLD ? 'rgba(241,196,15,.9)' : 'rgba(52,152,219,.9)';
  ...
  function openMovieDetail(movieId) {
    ...
    $('movie-detail-meta').innerHTML = `导演 ${escapeHtml(movie.director)} · 上映 ${escapeHtml(movie.releaseDate)} · 票房 ${formatBoxOffice(movie.boxOffice)}`;
    $('movie-detail-heat').innerHTML = `<span style="color:${getHeatColor(movie.heatScore)}">● 热度 ${Number(movie.heatScore)}/100 · ${getHeatLabel(movie.heatScore)}</span>`;
    ...
    select.innerHTML = '<option value="">不对比</option>' +
      (movie.matchedDirectorIds || []).map(id => {
        ...
        return d ? `<option value="${escapeHtml(id)}">${escapeHtml(d.name)} (${Math.round((movie.matchScores[id] || 0) * 100)}%)</option>` : '';
      }).join('');
    ...
    if (movie.signatureScenes && movie.signatureScenes.length > 0) {
      scenes.innerHTML = '<h3>标志性场景</h3><div class="movie-scenes-list">' +
  ...
  function openDirectorSwap(movieId) {
    ...
    grid.querySelectorAll('.swap-director-card').forEach(card => {
      ...
      card.onkeydown = (e) => {
        if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); selectDirectorForSwap(card.dataset.directorId); }
      };
    });
  ...
  function updateSwapPreview() {
    ...
    $('swap-info').innerHTML = `
      ...
      <div class="swap-color-preview">
        ${Object.entries(blendedColors).map(([k, hex]) => {
          const safeHex = sanitizeColor(String(hex), '#1a1a1a');
  ...
  function drawSwapDNARadar(movieDNA, directorDNA, blendedDNA) {
    ...
    const n = dims.length, angleStep = (Math.PI * 2) / n;
  ...
  function openQuoteCardGenerator(movieId) {
    ...
    list.innerHTML = movie.iconicQuotes.map((q, i) => `
  ...
  function initDNASliders(movie) {
    ...
    const dims = [
      ...
      { key: 'compositionType', label: '构图', options: ['symmetric', 'centered', 'asymmetric', 'dynamic'], labels: ['对称', '居中', '不对称', '动态'] },
      { key: 'lightingType', label: '光影', options: ['natural', 'high-key', 'low-key', 'dramatic'], labels: ['自然', '高调', '低调', '戏剧'] },
      ...
      { key: 'texture', label: '质感', options: ['smooth', 'digital', 'grainy', 'painterly', 'handdrawn'], labels: ['平滑', '数字', '颗粒', '绘画', '手绘'] }
    ];

    const currentDNA = movie.styleDNA || {};
    state.customDNA = { ...currentDNA };

    sliderList.innerHTML = dims.map(dim => {
  ...
  function openSceneRecreate(movieId) {
    ...
    list.innerHTML = movie.signatureScenes.map((scene, i) => `
  ...
  function recommendMoviesByEmotion() {
    ...
    const scored = state.movies.map(movie => {
  ...
  function loadChallenge() {
    ...
    lbList.innerHTML = state.challengeData.leaderboard.map(item => `
  ...
  function renderSeason(tab) {
    if (tab === 'current') {
      ...
      $('season-reward-list').innerHTML = rewards.map(r => `
